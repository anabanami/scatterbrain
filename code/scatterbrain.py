...
## ZERO BC r->0 , free r->Rmax (NON-HERMITIAN)
...
# ylabel = plt.ylabel(r'$V_Xe$ (meV) and $u_{i}$ (arb) ')
# plt.xlabel(r'$r$ (Å)')
# plt.grid(True)
...
# ## SHOOTING method
# ## ENERGY
# k = np.sqrt(-2 * M_red * -25 * meV) / hbar #initial guess E_binding = -25 meV
# ## ODE
# def ODE(y, r, k):
#     u, w = y
...
#     else:
#         result[0] = w
#         result[1] = - 1 / (4 * r**2) +  2 * M_red * V_Xe(r) / hbar**2 - k**2
#         return result
# ## ICS
# y_0 = [0, 1] # as per VPA

# ## ODEINT solver
# solution_ODE = odeint(ODE, y_0, r, args=(k,))

# # plt.plot(r, solution_ODE[:, 0])
# plt.title('Shooting method Schmidt boundstate')
# plt.grid(True)
# # plt.show()
